...
export default function FeedScreen() {
  ...
  const renderPost = ({ item: post }: { item: Post }) => {
    ...
    return (
      <View className="bg-slate-800 mx-4 mb-4 rounded-2xl overflow-hidden">
        {/* Author Header */}
        <Pressable
        ...
        >
          <View
            className={`w-12 h-12 rounded-full items-center justify-center ${
              post.authorRole === 'professional' ? 'bg-amber-500/20' : 'bg-blue-500/20'
            }`}
          >
            {post.authorPhotoUrl ? (
              <Image
          ...
          <View className="ml-3 flex-1">
            <View className="flex-row items-center">
              ...
              >
                <Text
                  className={`text-xs ${
                    post.authorRole === 'professional' ? 'text-amber-400' : 'text-blue-400'
                  }`}
                >
        ...
        <View className="px-4 pb-3">
          <View className="flex-row items-center mb-2">
            ...
            <View className="bg-slate-700/50 px-3 py-1 rounded-full ml-2">
        ...
        {/* Actions */}
        <View className="flex-row items-center px-4 py-3 border-t border-slate-700">
          <Pressable
        ...
        <View className="flex-row items-center justify-between">
            ...
            {(['all', 'clients', 'professionals'] as const).map((type) => (
              <Pressable
                ...
                className={`px-4 py-2 rounded-full mr-2 ${
                  filters.type === type ? 'bg-amber-500' : 'bg-slate-800'
                }`}
              >
                <Text
          ...
          <Pressable
            ...
            className={`flex-row items-center px-3 py-2 rounded-full ${
              filters.categoryId ? 'bg-amber-500' : 'bg-slate-800'
            }`}
          >
              ...
              {SERVICE_CATEGORIES.map((category) => (
                <Pressable
                  ...
                  className={`px-3 py-2 rounded-full mr-2 ${
                    filters.categoryId === category.id ? 'bg-amber-500' : 'bg-slate-700'
                  }`}
                >
                  <Text
                    className={`text-sm ${
      ...
      {filteredPosts.length === 0 ? (
        <View className="flex-1 items-center justify-center px-8">
          <View className="w-20 h-20 bg-slate-800 rounded-full items-center justify-center mb-4">
            <Briefcase color="#64748B" size={32} />
          </View>
